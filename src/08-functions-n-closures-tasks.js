...
function getPolynom(...args) {
  return (x) => args.reduce((prev, current, i) => prev + current * x ** (args.length - 1 - i), 0);
}
...
function logger(func, logFunc) {
  return function log(...args) {
    const argums = Array.from(args);
    let callString = JSON.stringify(argums);
    callString = callString.slice(1, -1);
