...
function isInsideCircle(circle, point) {
  ...
  return !((rad1 ** 2 + rad2 ** 2 >= circle.radius ** 2));
}
...
function isCreditCardNumber(ccn) {
  const cnnString = ccn.toString().split('').reverse();
  const arr = cnnString.map((item, i) => {
    if (i % 2 !== 0) {
      if (2 * item > 9) {
        return 2 * item - 9;
      }
  ...
  return arr.reduce((a, b) => a / 1 + b / 1) % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (index % 2 !== 0) {
      ...
      if (lastBrackets !== brackets[index - 1]) return false;
